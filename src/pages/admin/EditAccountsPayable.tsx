...
import { DollarSign, Building2, FileText, ArrowLeft } from 'lucide-react';
...
const EditAccountsPayable = () => {
  const navigate = useNavigate();
  const { accountId } = useParams();

  // Mock data - em uma aplicação real, isso viria de uma API
  const mockAccount: AccountsPayableItem = {
    id: parseInt(accountId || '1'),
    description: 'Compra de Tecidos - Fashion Outlet',
    amount: 2500.0,
    dueDate: '2024-02-20',
    status: 'pending',
    supplier: 'Fashion Outlet Ltda',
    supplierPhone: '(11) 33333-3333',
    paymentMethod: 'Transferência Bancária',
    notes: 'Compra de tecidos para nova coleção. Pagamento em 30 dias.',
    createdAt: '2024-01-15T10:30:00Z',
    updatedAt: '2024-01-20T14:45:00Z',
  };

  const [account] = useState<AccountsPayableItem>(mockAccount);
  ...
  const [supplier, setSupplier] = useState(mockAccount.supplier);
  const [supplierPhone, setSupplierPhone] = useState(mockAccount.supplierPhone);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!description.trim() || !supplier.trim() || !supplierPhone.trim() || !paymentMethod.trim()) {
      ...
      <div className='w-full max-w-7xl mx-auto'>
        <div className='mb-8'>
          ...
          <h1 className='text-2xl sm:text-3xl font-bold text-slate-800 mb-2'>
            Editar Conta a Pagar #{account.id}
          </h1>
          <p className='text-sm sm:text-base text-slate-600'>
            Modifique as informações da conta a pagar abaixo
          </p>
        </div>
        ...
        >
          ...
          <div className='p-6 bg-slate-50 rounded-lg border border-slate-200'>
            <div className='flex items-center gap-3 mb-4'>
              ...
              <h3 className='text-lg font-semibold text-slate-800'>Informações do Fornecedor</h3>
            </div>

            <div className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
              <div className='flex flex-col gap-2'>
                <label className='text-sm font-semibold text-slate-700' htmlFor='supplier'>
                  Nome do Fornecedor *
                </label>
                <input
                  id='supplier'
