...
const CashFlowSessionView = ({ session, movements, onClose, onEdit }: CashFlowSessionViewProps) => {
  ...
  return (
    <div className='fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-300'>
      <div className='bg-white rounded-2xl shadow-2xl w-full max-w-7xl max-h-[95vh] overflow-hidden border border-slate-100 animate-in slide-in-from-bottom-4 duration-300'>
        {/* Header */}
          ...
          <div className='relative flex items-center justify-between'>
            <div>
              <div className='flex items-center gap-3 mb-1'>
                ...
                <div>
                  <h2 className='text-xl font-bold'>Sessão de Caixa #{session.id}</h2>
            ...
            <div className='flex items-center gap-4'>
              <div className='flex flex-col items-end gap-1'>
                <span
                  className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(session.status)}`}
                >
        ...
        <div className='overflow-y-auto max-h-[calc(95vh-140px)]'>
          <div className='p-6'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-4 gap-3 mb-6'>
              <div className='bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-3 hover:shadow-lg transition-all duration-300'>
                ...
                <p className='text-xs text-blue-600 mt-1'>Valor inicial</p>
              </div>

              <div className='bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200 rounded-xl p-3 hover:shadow-lg transition-all duration-300'>
                <div className='flex items-center justify-between mb-2'>
              ...
              <div className='bg-gradient-to-br from-red-50 to-red-100 border border-red-200 rounded-xl p-3 hover:shadow-lg transition-all duration-300'>
                <div className='flex items-center justify-between mb-2'>
                ...
                <p className='text-xl font-bold text-red-900'>R$ {session.totalExits.toFixed(2)}</p>
                <p className='text-xs text-red-600 mt-1'>Total pago</p>
              </div>

              <div className='bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200 rounded-xl p-3 hover:shadow-lg transition-all duration-300'>
                <div className='flex items-center justify-between mb-2'>
                ...
                <p className='text-xs text-slate-600 mt-1'>Saldo final</p>
              </div>
            </div>
            ...
            <div className='grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6'>
                ...
                <div className='bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200 rounded-xl p-4'>
                  ...
                  <div className='space-y-3'>
                    ...
                    <div className='flex items-center gap-3 p-2 bg-white/50 rounded-lg'>
                      ...
                      <div>
                        <p className='text-sm font-medium text-slate-700'>Aberto por</p>
                        <p className='text-sm text-slate-600'>{session.openedBy}</p>
                      </div>
                    </div>
                    ...
                    <div className='flex items-center gap-3 p-2 bg-white/50 rounded-lg'>
                      ...
                      <div>
                        <p className='text-sm font-medium text-slate-700'>Aberto em</p>
                        <p className='text-sm text-slate-600'>
                    ...
                    {session.closedBy && (
                      <div className='flex items-center gap-3 p-2 bg-white/50 rounded-lg'>
                        ...
                        <div>
                          <p className='text-sm font-medium text-slate-700'>Fechado por</p>
                          <p className='text-sm text-slate-600'>{session.closedBy}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {session.closedAt && (
                      <div className='flex items-center gap-3 p-2 bg-white/50 rounded-lg'>
                        ...
                        <div>
                          <p className='text-sm font-medium text-slate-700'>Fechado em</p>
                          <p className='text-sm text-slate-600'>
              ...
              <div className='lg:col-span-2'>
                <div className='bg-white border border-slate-200 rounded-xl shadow-sm'>
                  <div className='p-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-slate-100 rounded-t-xl'>
                    <div className='flex items-center justify-between'>
                      ...
                      <div className='flex items-center gap-3'>
                        ...
                        <div className='flex items-center gap-2 text-sm'>
                          ...
                          <span className='text-red-700 font-medium'>Saídas</span>
                        </div>
                      </div>
                    </div>
                  </div>
                        ...
                        {movements.length > 0 ? (
                          movements.map((movement, index) => (
                            ...
                            >
                              ...
                              <td className='px-4 py-3 text-center'>
                                <span className='text-sm font-medium text-slate-700 bg-slate-100 px-2 py-1 rounded-full'>
                          ...
                          <tr>
                            <td colSpan={6} className='px-4 py-8 text-center'>
                              <div className='flex flex-col items-center gap-3'>
                                ...
                                <div>
                                  <p className='text-slate-500 font-medium'>Nenhuma movimentação</p>
            ...
            {session.notes && (
              <div className='bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-4 mb-6'>
                <div className='flex items-start gap-3'>
                  ...
                  <div>
                    <h3 className='text-lg font-bold text-amber-800 mb-2'>Observações</h3>
                    <p className='text-amber-700 leading-relaxed'>{session.notes}</p>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className='flex gap-4 p-6 border-t border-slate-200 bg-slate-50'>
          <button
            onClick={onClose}
            className='flex-1 py-3 px-6 border border-slate-300 text-slate-700 font-semibold rounded-xl hover:bg-white hover:shadow-md transition-all duration-300'
          >
            Fechar
          </button>
          {onEdit && session.status === 'open' && (
            <button
              onClick={onEdit}
              className='flex-1 py-3 px-6 bg-gradient-to-r from-purple-600 to-pink-500 text-white font-semibold rounded-xl hover:from-purple-700 hover:to-pink-600 transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl'
            >
              Editar Sessão
            </button>
          )}
